                ...
                <motion.div key="m365-list" {...fadeUp}>
                  ...
                  <div className="flex flex-wrap gap-2 mb-5">
                    {M365_SEGMENTS.map(seg => {
                      const m365Labels = { all: tr.allPlans, business: 'Business ≤300', enterprise: 'Enterprise', frontline: tr.frontlinePlans, features: lang === 'fr' ? 'Disponibilités Features' : 'Feature Availability' };
                      return (
                  ...
                  {m365Seg === 'features' && (
                    <motion.div key="features-view" {...fadeUp}>
                      <p className="text-sm text-gray-500 mb-5">
                        {lang === 'fr'
                          ? '9 catégories · 45+ outils · disponibilité par plan M365'
                          : '9 categories · 45+ tools · M365 plan availability'}
                      </p>
                      <div className="space-y-4">
                        {M365_TOOL_CATEGORIES.map((cat) => (
                          <div key={cat.id} className={`rounded-2xl border ${cat.border} overflow-hidden`}>
                            {/* Category header */}
                            <div className={`bg-gradient-to-r ${cat.gradient} px-5 py-3 flex items-center gap-3`}>
                              <span className="text-white font-black text-lg w-8 h-8 bg-white/20 rounded-xl flex items-center justify-center shrink-0">{cat.letter}</span>
                              <div>
                                <h3 className="text-white font-bold text-sm">{cat.label}</h3>
                                <p className="text-white/70 text-xs">{cat.sublabel}</p>
                              </div>
                            </div>
                            {/* Tools grid */}
                      ...
                      <div className="mt-5 p-4 bg-white rounded-2xl border border-gray-100">
                        <p className="text-xs font-bold text-gray-600 mb-2">{lang === 'fr' ? 'Légende disponibilité :' : 'Availability legend:'}</p>
                        <div className="flex flex-wrap gap-2">
                          {Object.entries(AVAIL).map(([k, s]) => (
                            <span key={k} className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${s.bg} ${s.text} ${s.border}`}>{s.label}</span>
                          ))}
                        </div>
                        <p className="text-[10px] text-gray-400 mt-2">{lang === 'fr' ? '* "Add-on" = coût additionnel, non inclus dans le plan de base' : '* "Add-on" = additional cost, not included in base plan'}</p>
                      </div>
                    </motion.div>
                  )}

                  {m365Seg !== 'features' && <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
